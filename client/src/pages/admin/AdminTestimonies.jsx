...
const CAT_COLORS = {
  harcelement_verbal: HS.warn, agression_physique: HS.danger, vol: '#5A6B8A',
  suivi: HS.aloewood, detour_force: '#C97B3B', autre: HS.milkTea,
};
    ...
    <PageShell>
      <div style={{ padding: '54px 20px 16px', display: 'flex', alignItems: 'center', gap: 14,
        ...
        <div style={{ flex: 1 }}>
          ...
          <H2 style={{ marginTop: 2 }}>Témoignages en attente</H2>
        </div>
        {testimonies.length > 0 && (
          <span style={{ background: HS.sakura, color: HS.chocolate, borderRadius: 10,
            padding: '2px 8px', fontSize: 11, fontWeight: 800 }}>{testimonies.length}</span>
        )}
      </div>
